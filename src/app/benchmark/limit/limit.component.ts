...
export class LimitComponent implements OnInit {
  ...
  constructor(private benchmarkService: BenchmarkService) {
    ...
    this.chartOptions = LimitComponent.CHART_UTIL.getCommonChartOptions();
  ...
  private loadChartData() {
      ...
      this.chart.addSeries(
        {
          ...
          color: LimitComponent.CHART_UTIL.getSeriesColor(this.chartOptions.series[i].name),
    ...
    LimitComponent.CHART_UTIL.addChartLabel(
      this.chart,
    ...
    LimitComponent.CHART_UTIL.addChartImage(this.chart, 'https://www.advisen.com/img/advisen-logo.png', this.chart.chartWidth - 80, this.chart.chartHeight - 20, 69, 17);
  }
    ...
    for (i = 0; i < n1; i++) {
      ...
      for (j = 0; j < n2; j++) {
        if (group[this.chartOptions.xAxis.categories[j]]) {
          if (series.name === LimitComponent.CLIENT_LINE) {
